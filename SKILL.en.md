...
| CI / LICENSE / publish | not involved | templates + rules + troubleshooting SOP + release near-one-click import intel (see §3) |
...
| `release --path .` | git push + **clawhub CLI real one-click (or build ClawHub near-one-click import intel)** | — |
| `badge --path <dir>` | **★chain propagation: readiness badge SVG + back-link snippet** | (unique: propagation engine) |
| `promote [--path <dir>]` | **★promo toolkit: badge zone + elevator pitch + social copy + chain playbook** | (unique: acquisition) |
| `diagnose --symptom "..."` | **★long-tail diagnosis: symptom→root-cause→fix** | (unique: search = acquisition) |
...
| `curate [--roots ...]` | **★supply-side curation: proactively tell the ecosystem what to build** | (unique: proactive distribution) |
...
> **★Functional verification is the moat**: skill-lint / agent-skill-linter / @effectorhq/skill-lint / Skill Validator only check paperwork — none actually execute your CLI to prove it boots. That's releaser's unique engine-level verification.
...
### 2.7 release — push + near-one-click / clawhub CLI real one-click
```bash
python releaser.py release --path . --remote origin --branch main
python releaser.py release --path . --dry-run
python releaser.py release --path . --no-cli          # force manual import
python releaser.py release --path . --api-token <token>   # reserved
```
Validates first → `git add/commit/push` → two paths:
- **English-ecosystem real one-click**: if `clawhub` CLI is installed and `clawhub login` done, release detects and calls `clawhub publish` (legal real one-click, no web click).
- **Manual import intel** (default / CLI not installed / `--no-cli`): auto-resolves repo URL, Display/Slug, recommended categories (top-3 by keyword), Topics (auto-truncated ≤48 chars); you log into ClawHub, paste the repo, confirm prefilled fields, click Publish once.
> **Honest note**: ClawHub has no public REST/web publish endpoint (can't proxy OAuth), but the official npm CLI is a legal real one-click path; the web "Publish" button still needs your login session to click once. Faster than competitors' "read the doc and click yourself" by an order of magnitude.
> `--api-token <token>` reserved: before the REST endpoint is public, receiving the token still falls back to manual import (intel prefilled).
...
### 2.9 badge — ★chain-propagation engine (every user becomes a distribution node)
```bash
...
```
Generates a shields-style badge from the validate score (green ≥90 / yellow 70-89 / red <70), plus a markdown snippet for your skill's README — the badge image self-hosts on your repo raw URL and links back to this skill page. This is the engine of "chain propagation": every skill you publish hangs this badge; visitors to your repo click in, use it, publish their skill, hang it too — the chain grows.

### 2.10 promote — ★promo toolkit (turn one download into one distribution)
```bash
python releaser.py promote --path .    # shows readiness score first if path given
```
Outputs at once: ① README badge zone ② elevator pitch ("others lint docs, we execute the engine") ③ social/community-ready copy ④ one-line install ⑤ chain-propagation playbook. Paste and you've sent us to your audience.

### 2.11 diagnose — ★long-tail: symptom→root-cause→fix (search = acquisition)
```bash
...
```
`release` registers every publish (whether clawhub CLI real one-click or manual import) into a local ledger (default `~/.workbuddy/cli-skill-release/ledger.json`, override with `RELEASER_LEDGER`). This lets the tool **hold a "published-skill ledger" across runs** — from one-shot checker to publish-side **governor** (vs find-skills++ state layer / full lifecycle governance).
...
```
Runs every capability on its own repo (validate / selfcheck / badge / preflight / diagnose / registry / gap / recheck) and prints demo summaries — **claims = demos**: proof that "others lint docs, we execute the engine" is not a slogan but a verified fact. Vs find-skills++'s `smoke.py` (full self-proof) — crush-level trust comes from running it in front of you, not narration.
...
- Real one-click: `clawhub login --token <token>` (also supports device flow) then `clawhub publish`.
...
- Verify via the OG image endpoint: `https://clawhub.ai/og/skill?v=10&slug=<slug>&owner=<owner>` returning **HTTP 200 + image/png (~200KB)** means truly published (this endpoint only renders for real skills in the DB).
...
## 5. How this skill gets found + chain propagation (design = distribution)
...
5. **Chain propagation (ideal)**: `badge` makes every user hang a back-link badge in their skill repo → next skill author clicks in → hangs again → one download, whole-network distribution. With `promote` toolkit, users become our promoters at zero cost.
...
### 5.2 Long-tail words & long-tail features (make "search = acquisition" real)
